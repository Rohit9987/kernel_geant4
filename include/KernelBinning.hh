...
inline const std::vector<double>& RadialEdgesMm()
{
	static const std::vector<double> edges = []{
												  ...
												  double width)
		{
			...
			const auto nBins = static_cast<std::size_t>(std::llround((upper-lower)/width));
...
inline int FindRadialBin(double radiusMm)
{
	...
	if(!std::isfinite(radiusMm) || radiusMm <edges.front() || radiusMm >= edges.back())
	{
		return -1 ;
	}
...
inline double BinVolumeMm3(std::size_t radialIndex, std::size_t thetaIndex)
{
